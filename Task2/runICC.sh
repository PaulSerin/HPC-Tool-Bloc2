...
#SBATCH --job-name=spmv_benchmark_icc
#SBATCH --output=spmv_benchmark_output_icc.out

# Load the ICC module
module purge
module load intel

# List of optimization levels and corresponding CFLAGS for ICC
optimization_levels=("O0" "O2-novec" "O3-vec" "Ofast" "Ref")
cflags_list=("-O0" "-O2 -no-vec" "-O3 -xHost" "-Ofast" "-O2")
...
for i in "${!optimization_levels[@]}"; do
    ...
    echo "==========================================="
    echo "Compiling and running with ICC optimization: $opt_level"
    echo "CFLAGS: $cflags"
    echo "==========================================="
    ...
    make CC=icc CFLAGS="-Wall -Wextra $cflags"

    if [ $? -ne 0 ]; then
        echo "Compilation failed with ICC CFLAGS: $cflags"
        exit 1
    fi
    ...
    if [ $? -ne 0 ]; then
        echo "Execution failed for ICC optimization level $opt_level"
        echo "$output"
        continue
    fi
...
# Now, print the results in a table
echo
echo "Benchmark Results for ICC"
echo "========================="
printf "%-10s" "Function"
